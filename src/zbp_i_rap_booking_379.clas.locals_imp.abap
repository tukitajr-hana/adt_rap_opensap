...
CLASS lhc_Booking IMPLEMENTATION.
  ...
  METHOD calculateBookingID.
    ...
    DATA lf_update TYPE TABLE FOR UPDATE ZI_RAP_atrav_379\\Booking.
    ...
    " Process all affected Travels. Read respective bookings,
    " determine the max-id and update the bookings without ID.
    LOOP AT travels INTO DATA(travel).
      ...
      " Select max Booking ID -- TESTE !?!
      SELECT MAX( BookingID )
      FROM Zi_RAP_ABOOK_379
      WHERE TravelUUID = @travel-TravelUUID
       INTO @DATA(max_BookingID2).
